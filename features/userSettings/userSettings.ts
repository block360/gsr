...
export type SaveUserSettingsFunction = (
  slippageInput: BigNumber
) => Observable<boolean>;
...
function saveSettings(
  ...
  return saveUserSettings$(slippageInput)
    .pipe(
      map(
        () => ({ kind: "settingsSaved", slippageInput } as UserSettingsChange)
      ),
      startWith({ kind: "stage", stage: "inProgress" } as UserSettingsChange),
      catchError(() =>
        of({ kind: "stage", stage: "failure" } as UserSettingsChange)
      )
    )
...
function addTransitions(
  ...
  return {
    ...state,
    saveSettings: () =>
      saveSettings(state.slippageInput, change, saveUserSettings$),
  };
}
...
function validate(state: UserSettingsState): UserSettingsState {
  ...
  const invalidSlippage =
    ...
    slippageInput.isNaN() ||
    (slippageInput &&
      (slippageInput.gt(SLIPPAGE_LIMIT_MAX) ||
        slippageInput.lt(SLIPPAGE_LIMIT_MIN)));
...
export function createUserSettings$(
  ...
  return checkUserSettings$().pipe(
    switchMap(({ slippage: initialSlippage }) => {
      ...
      const slippage = initialSlippage
        ? new BigNumber(initialSlippage)
        : SLIPPAGE_DEFAULT;
