...
export function ModalCloseIcon({
  ...
  const handleEscClose = useCallback((event: any) => {
    const { keyCode } = event;
    if (keyCode === 27) {
      close();
    }
...
export function MobileSidePanel({
  ...
  const { vaultFormOpened, setVaultFormOpened, setVaultFormToggleTitle } =
    useSharedUI();
    ...
    <MobileSidePanelPortal>
      <Box
        sx={{
          ...
          transform: [
            `translateX(${vaultFormOpened ? "0" : "100"}%)`,
            "translateX(0)",
          ],
...
export const MODAL_CONTAINER_TREZOR_METAMASK_EIP1559 =
  "trezor-metamask-eip1559";
...
export function ModalTrezorMetamaskEIP1559() {
  ...
  function close() {
    const modal = document.getElementById(
      MODAL_CONTAINER_TREZOR_METAMASK_EIP1559
    );
      ...
      <Grid sx={{ p: 4, fontSize: 2 }}>
        ...
        <Box>
          <Trans
            ...
            components={[
              <AppLink sx={{ fontSize: "inherit" }} href="/inprogress" />,
            ]}
          />
        </Box>
        <Box>
